...
interface AttorneyLoginResponse {
  success: boolean;
  message: string;
  token: string;
  user: {
...
export default function AttorneyLogin() {
  ...
  useEffect(() => {
    ...
    const checkAuth = () => {
      ...
      if (authed && userType === "attorney") {
        console.log("✅ Already authenticated as attorney - redirecting");
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      login(response.token, {
        ...
        lawFirmName: response.user.lawFirmName,
        phoneNumber: response.user.phoneNumber,
        verified: response.user.verified,
        verificationStatus: response.user.verificationStatus,
        tierLevel: response.user.tierLevel,
      });
    ...
    >
      ...
      <aside className="hidden lg:flex flex-col w-[280px] bg-[#0A2342] text-white relative">
        ...
        <div className="px-6 py-8 mt-48">
          <h2 className="text-lg font-semibold mb-3">Attorney Login</h2>
          <p className="text-sm text-blue-100">
            Access your attorney dashboard, manage cases, and view war rooms.
          </p>
        </div>
        ...
        <div className="px-6 py-4 mt-auto space-y-3 text-sm text-blue-100">
          <div className="flex items-start gap-2">
            ...
            <span>Manage your cases</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-green-400">✓</span>
            <span>Access war rooms</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-green-400">✓</span>
            <span>Review juror applications</span>
          </div>
        </div>
      </aside>
      ...
      <section className="flex-1 flex flex-col relative px-6 sm:px-12 py-10">
        ...
        {/* Login Form - CHANGED: Removed justify-center, added ml-0 lg:ml-16 */}
