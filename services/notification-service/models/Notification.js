...
async function markAsUnread(notificationId) {
  try {
    const id = parseInt(notificationId, 10);
    if (isNaN(id) || id <= 0) throw new Error("Valid notification ID is required");
    const pool = await poolPromise;
    await pool.request().input("notificationId", sql.Int, id).query(`
      UPDATE dbo.Notifications SET IsRead = 0, ReadAt = NULL
...
async function getNotificationStats(userId, userType) {
  try {
    const id = parseInt(userId, 10);
    const pool = await poolPromise;
    const result = await pool
      ...
      .query(`
        ...
        FROM dbo.Notifications
        WHERE (@userType = 'admin' OR UserId = @userId) AND UserType = @userType
      `);
...
async function deleteReadNotifications(userId, userType) {
  try {
    const id = parseInt(userId, 10);
    const pool = await poolPromise;
    const result = await pool
      ...
      .query(`
        DELETE FROM dbo.Notifications
        WHERE (@userType = 'admin' OR UserId = @userId) AND UserType = @userType AND IsRead = 1;
        SELECT @@ROWCOUNT as affected;
      `);
    return result.recordset[0].affected;
  } catch (error) {
