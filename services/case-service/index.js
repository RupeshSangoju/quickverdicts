...
app.get("/health", async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, service: "case-service", status: "ERROR" });
  }
});
...
app.use("/witnesses", witnessRoutes);
app.use("/tier-upgrades", tierUpgradeRoutes);
...
async function start() {
  try {
    const pool = await poolPromise;
    await pool.request().query("SELECT 1");
    console.log("✅ case-service: database connected");

    const PORT = process.env.PORT || 5004;
    http.createServer(app).listen(PORT, "0.0.0.0", () => {
      console.log(`🚀 case-service running on port ${PORT}`);
      if (!process.env.AZURE_STORAGE_CONNECTION_STRING) {
        console.warn("⚠️  AZURE_STORAGE_CONNECTION_STRING not set — document SAS URLs will not work");
      }
    });
  } catch (err) {
    console.error("❌ case-service failed to start:", err);
    process.exit(1);
  }
}
