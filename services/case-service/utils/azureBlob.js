...
// Validate connection string — non-fatal; Azure features degrade gracefully
if (!AZURE_STORAGE_CONNECTION_STRING) {
  console.warn("⚠️  AZURE_STORAGE_CONNECTION_STRING not configured — blob storage disabled");
} else if (
  !AZURE_STORAGE_CONNECTION_STRING.includes("AccountName=") ||
  !AZURE_STORAGE_CONNECTION_STRING.includes("AccountKey=")
) {
  console.warn("⚠️  Invalid AZURE_STORAGE_CONNECTION_STRING format — blob storage disabled");
}
...
try {
  if (!AZURE_STORAGE_CONNECTION_STRING) throw new Error("not configured");
